...
from    .utilities          import  auto_repr
...
@auto_repr
class AccessSetup:
    '''Structure storing constant attributes of an ACCES optimisation run.
...
@auto_repr
class AccessPaths:
    '''Structure storing all paths relevant to an ACCES optimisation run.
...
class Access:
    ...
    def print_finished(self, es, scaling):
        solutions = np.append(es.result.xbest * scaling, es.result.fbest)
...
class AccessData:
    ...
    def legacy(access_path):
        ...
        notfound.__repr__ = lambda self: "FileNotFoundLegacy"
        notfound = notfound()
